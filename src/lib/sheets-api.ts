...
import { sheetGet, sheetPost, sheetPublicPost, rowsOf } from "./sheets-gateway";
...
/** Busca de UM pedido para telas administrativas, sempre atrás da sessão admin. */
export async function fetchOrderByIdAdmin(id: string): Promise<StoredOrder | undefined> {
  const normalized = String(id || "").trim();
  if (!/^[A-Za-z0-9_-]{8,100}$/.test(normalized)) return undefined;
