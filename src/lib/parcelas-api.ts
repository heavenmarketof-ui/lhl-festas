...
export const STATUS_LABEL: Record<ParcelaStatus, string> = {
  a_gerar: "A GERAR", gerado: "GERADO", enviado: "ENVIADO", pago: "PAGO", vencido: "VENCIDO",
};
...
export async function cancelarPlanoParcelas(contratoId: string, contratoCliente: string): Promise<Parcela[]> {
  const atuais = await listarParcelas(contratoId);
  const pagas: ParcelaInput[] = atuais
    .filter((p) => p.status === "pago")
    .map((p) => ({
      id: p.id, numero: p.numero, total: p.total, valor: p.valor,
      vencimento: p.vencimento, status: "pago", observacoes: p.observacoes,
    }));
  return salvarParcelas(contratoId, contratoCliente, pagas);
}
...
export function gerarPlanoParcelas(opts: { quantidade: number; valorTotal: number; primeiroVencimento: string }): ParcelaInput[] {
  ...
  return Array.from({ length: qtd }, (_, i) => {
    const valorCentavos = base + (sobra > 0 ? 1 : 0); if (sobra > 0) sobra--;
    ...
    return { numero: i + 1, total: qtd, valor: valorCentavos / 100, vencimento, status: "a_gerar", observacoes: "" };
  });
}
...
export async function registrarPagamentoParcela(opts: { parcela: Parcela; contratoCliente: string; valorPago: number; data?: string }): Promise<Parcela> {
  ...
  if (!atuais.some((l) => l.id === lancamentoId)) {
    const lancamento: Lancamento = {
      ...
      contratoId: opts.parcela.contratoId, origem: `boleto_parcela:${opts.parcela.id}`,
      createdAt: new Date().toISOString(), ativo: "Sim",
    };
