    ...
    return (
      <Link
        ...
        className={`group flex items-center gap-3 rounded-2xl px-3 py-3 transition ${
          isActive ? "bg-white text-[#5a111d] shadow-sm" : "text-white/76 hover:bg-white/10 hover:text-white"
        }`}
      >
        <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl ${isActive ? "bg-[#f7ded9]" : "bg-white/8"}`}>
          <Icon className="h-4 w-4" />
        </span>
        <span className="min-w-0">
          <span className="block text-sm font-semibold leading-tight">{item.label}</span>
          {"desc" in item && <span className={`mt-0.5 block text-[11px] ${isActive ? "text-[#79585d]" : "text-white/46"}`}>{item.desc}</span>}
        </span>
      </Link>
    );
  };
  ...
  return (
    <div id="admin-app" className="min-h-screen bg-[#f8f5f1] text-[#3f2529] lg:grid lg:grid-cols-[270px_minmax(0,1fr)]">
      <aside className="hidden min-h-screen flex-col bg-gradient-to-b from-[#4a0d18] via-[#651421] to-[#3c0911] p-4 text-white lg:sticky lg:top-0 lg:flex lg:h-screen">
        <Link to="/admin" className="mb-5 flex items-center gap-3 rounded-2xl px-2 py-2">
          <img src={logo} alt="LHL Festas" className="h-12 w-12 rounded-full border border-[#d7ad68]/45 object-cover" />
          <div>
            <p className="font-serif text-xl leading-none text-[#f2d091]">LHL Festas</p>
            <p className="mt-1 text-[10px] uppercase tracking-[.18em] text-white/45">Admin Oficial</p>
          </div>
        </Link>
        ...
        <div className="mt-auto border-t border-white/10 pt-4">
          {email && <p className="mb-3 truncate px-3 text-[11px] text-white/45">{email}</p>}
          <div className="grid grid-cols-2 gap-2">
            <Button asChild variant="ghost" className="h-10 justify-start rounded-xl text-xs text-white/65 hover:bg-white/10 hover:text-white">
              <Link to="/"><Eye className="mr-2 h-4 w-4" /> Site</Link>
            </Button>
            <Button
              variant="ghost"
              className="h-10 justify-start rounded-xl text-xs text-white/65 hover:bg-white/10 hover:text-white"
        ...
        <header className="sticky top-0 z-40 border-b border-[#eadfd8] bg-[#fffaf6]/94 backdrop-blur lg:hidden">
          <div className="flex h-16 items-center justify-between px-4">
            <Link to="/admin" className="flex items-center gap-2">
              <img src={logo} alt="LHL Festas" className="h-10 w-10 rounded-full object-cover" />
              <div><p className="font-serif text-lg leading-none text-[#651421]">LHL Festas</p><p className="text-[10px] text-[#8a7074]">Admin</p></div>
            </Link>
            <button onClick={() => setMobileOpen((v) => !v)} className="grid h-10 w-10 place-items-center rounded-xl border border-[#e8d8d1] bg-white" aria-label="Abrir menu">
              {mobileOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
            </button>
          </div>
          {mobileOpen && (
            <div className="border-t border-[#eadfd8] bg-[#5b111d] p-3">
              <nav className="grid gap-1 sm:grid-cols-2">
                {nav.map((item) => <NavItem key={item.label} item={item} />)}
                {secondary.map((item) => <NavItem key={item.label} item={item} />)}
              </nav>
              <div className="mt-2 flex gap-2 border-t border-white/10 pt-3">
                <Button asChild variant="ghost" className="flex-1 text-white/70 hover:bg-white/10 hover:text-white"><Link to="/"><Eye className="mr-2 h-4 w-4" /> Ver site</Link></Button>
                <Button variant="ghost" className="flex-1 text-white/70 hover:bg-white/10 hover:text-white" onClick={() => signOutAdmin().then(() => navigate({ to: "/auth", replace: true }))}><LogOut className="mr-2 h-4 w-4" /> Sair</Button>
              </div>
            </div>
          )}
        </header>

        <main className="min-h-screen bg-[radial-gradient(circle_at_top_right,rgba(216,121,130,.10),transparent_28%),#f8f5f1]">
...
export function orderCalendarLevel(
  ...
  if (opAtual) {
    ...
    if (opAtual.kitProntoConfirmadoEm) return "purple";
...
export function PriorityDot({ level }: { level: CalendarLevel | null }) {
  ...
  return <span className={`inline-block h-2.5 w-2.5 rounded-full ${DOT_CLASS[level] ?? "bg-muted"}`} title={DOT_LABEL[level] ?? ""} aria-label={DOT_LABEL[level] ?? ""} />;
}
