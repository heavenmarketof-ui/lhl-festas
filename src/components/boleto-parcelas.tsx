...
import { CheckCircle2, FileText, Send, WalletCards, Trash2 } from "lucide-react";
...
export function BoletoParcelas({contratoId,cliente,saldoPendente,dataEvento,onPagamentoRegistrado}:{contratoId:string;cliente:string;saldoPendente:number;dataEvento?:string;onPagamentoRegistrado?:()=>void}){
 ...
 useEffect(()=>{
   ...
   Promise.all([fetchOrderByIdPublic(contratoId),fetchLancamentos({force:true})]).then(([order,lanc])=>{
     ...
     // O dashboard financeiro só reconhece A Receber após a venda. O boleto,
     // porém, precisa enxergar o saldo negociado que será parcelado. Antes do
     // primeiro recebimento, respeita o "Valor Restante" informado no contrato.
     const calculado=p.vendaConfirmada?p.saldoNegociado:(restanteDeclarado>0?restanteDeclarado:p.saldoNegociado);
     setSaldoParcelavel(Math.max(0,calculado));
   }).catch(()=>{});
   return()=>{vivo=false};
 },[contratoId,saldoPendente]);

 useEffect(()=>{if(saldoParcelavel<=.009){setParcelas([]);setDraft([]);return}const timer=window.setTimeout(()=>void carregar(),250);return()=>window.clearTimeout(timer)},[contratoId,saldoParcelavel]);
 const total=useMemo(()=>draft.reduce((s,p)=>s+Number(p.valor||0),0),[draft]);const quitado=saldoParcelavel<=.009;const temPendentes=parcelas.some(p=>p.status!=="pago");
