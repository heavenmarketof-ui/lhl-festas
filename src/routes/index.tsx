...
function Header(){
  ...
  return <header className="relative z-50 border-b border-[#eadbd5]/70 bg-[#fffaf6] text-[#40272a]">
    <div className="mx-auto flex h-[74px] max-w-[1500px] items-center justify-between px-[clamp(18px,4vw,64px)] lg:h-[82px]">
      ...
      <nav className="hidden items-center gap-[clamp(18px,2vw,30px)] text-[13px] lg:flex">{nav.map((n,i)=><a key={n} href={href(i)} className={i===0?"text-[#d55f73] underline decoration-2 underline-offset-[11px]":"transition hover:text-[#d55f73]"}>{n}</a>)}<Link to="/catalogo" className="font-semibold text-[#b95063] transition hover:text-[#d55f73]">Catálogo</Link></nav>
    ...
    {o&&<div className="absolute inset-x-0 top-full bg-[#fffaf6] p-5 shadow-xl lg:hidden">{nav.map((n,i)=><a key={n} onClick={()=>setO(false)} href={href(i)} className="block py-3 text-sm">{n}</a>)}<Link to="/catalogo" onClick={()=>setO(false)} className="block py-3 text-sm font-semibold text-[#b95063]">Catálogo</Link><Link to="/orcamento" onClick={()=>setO(false)} className="mt-2 flex justify-center rounded-full bg-[#d76578] px-5 py-3 text-white">Fazer Orçamento</Link></div>}
  </header>
}
...
function Footer(){return <footer id="contato" className="border-t border-[#eadbd5] bg-[#fffaf6] py-8"><div className="mx-auto flex max-w-[1440px] flex-col items-center justify-between gap-5 px-[clamp(18px,4vw,60px)] sm:flex-row"><div className="flex items-center gap-5"><img src={logoImages[0]} alt="LHL Festas" className="h-12"/><span className="text-xs text-[#735f63]">Decorações que celebram a vida.</span></div><div className="flex flex-wrap items-center justify-center gap-4 text-xs"><a href="#inicio">Início</a><a href="#modalidades">Nossas Festas</a><Link to="/catalogo" className="font-semibold text-[#b95063]">Catálogo</Link><a href="#como">Como Funciona</a><a href={WA}>Contato</a><a href="https://www.instagram.com/lhl_festas/" target="_blank" rel="noreferrer"><Instagram className="h-4 w-4"/></a></div><p className="font-serif text-xl italic text-[#d26b7c]">Festas<br/>que ficam<br/>pra sempre ♥</p></div></footer>}
