...
function AgendaPage() {
  ...
  const resumo = useMemo(() => {
    ...
    const saldoMes = festasMes.reduce((s, o) => s + resumoFinanceiroContrato(o, idx).saldo, 0);
  ...
  }, [orders, cursor, todayISO, in7ISO, gateByOrder, aguardandoSinalList, idx]);
    ...
    <AdminShell>
      <div className="mx-auto w-full max-w-[1500px] space-y-6 px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-6">
          <div className="rounded-3xl border border-[#eaded8] bg-white p-5 shadow-sm"><div className="text-[10px] font-semibold uppercase tracking-[.16em] text-[#a18479]">Contratos no mês</div><div className="mt-2 font-serif text-4xl text-[#651421]">{resumo.total}</div></div>
          <div className="rounded-3xl border border-[#d9e8dc] bg-white p-5 shadow-sm"><div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[.16em] text-[#667b6a]"><ShieldCheck className="h-4 w-4" /> Operação liberada</div><div className="mt-2 font-serif text-4xl text-[#651421]">{resumo.liberadas}</div></div>
          <div className="rounded-3xl border border-[#f0dfc4] bg-white p-5 shadow-sm"><div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[.16em] text-[#9a7541]"><WalletCards className="h-4 w-4" /> Aguardando sinal</div><div className="mt-2 font-serif text-4xl text-[#651421]">{resumo.aguardandoSinal}</div></div>
          <div className="rounded-3xl border border-[#eaded8] bg-white p-5 shadow-sm"><div className="text-[10px] font-semibold uppercase tracking-[.16em] text-[#a18479]">Próximos 7 dias</div><div className="mt-2 font-serif text-4xl text-[#651421]">{resumo.proximas7}</div></div>
          <div className="rounded-3xl border border-rose-100 bg-white p-5 shadow-sm"><div className="text-[10px] font-semibold uppercase tracking-[.16em] text-rose-700">Saldo a receber no mês</div><div className="mt-2 font-serif text-xl text-rose-800">{fmtBRL(resumo.saldoMes)}</div></div>
          <div className="rounded-3xl border border-[#eaded8] bg-white p-5 shadow-sm"><div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[.16em] text-[#a18479]"><CalendarDays className="h-4 w-4 text-[#d87982]" /> Legenda</div><CalendarLegend /></div>
        </div>

        {aguardandoSinalList.length > 0 && !loading && (
          <section className="rounded-3xl border border-[#ecd9b8] bg-[#fff9ed] p-4 sm:p-5">
            <div className="mb-3 flex items-center gap-2 text-sm font-semibold text-[#815f2d]"><WalletCards className="h-4 w-4" /> Aguardando confirmação de recebimento</div>
            <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
              {aguardandoSinalList.map((order) => {
                const data = toDateISO(order.details?.dataEvento);
                const fin = resumoFinanceiroContrato(order, idx);
                return (
                  <button key={order.id} type="button" className="rounded-2xl border border-[#ead8b6] bg-white p-3 text-left transition hover:border-[#c89b58] hover:shadow-sm" onClick={() => navigate({ to: "/admin/$id", params: { id: order.id } })}>
                    <div className="flex items-start justify-between gap-2"><div><div className="font-medium text-[#651421]">{order.nome || "Cliente não informado"}</div><div className="mt-1 text-xs text-[#806e67]">{order.tema || "Tema não informado"}</div></div><span className={`rounded-full border px-2 py-1 text-[9px] font-semibold ${classeStatusFinanceiro(fin.status)}`}>{fin.status}</span></div>
                    <div className="mt-2 text-xs font-medium text-[#9a7541]">Festa: {data ? data.split("-").reverse().join("/") : "sem data"}</div>
                    <div className="mt-2 grid grid-cols-2 gap-2 text-[10px]"><span className="rounded-lg bg-emerald-50 p-2 text-emerald-800">Recebido<br/><b>{fmtBRL(fin.recebido)}</b></span><span className="rounded-lg bg-rose-50 p-2 text-rose-800">Saldo<br/><b>{fmtBRL(fin.saldo)}</b></span></div>
                  </button>
                );
              })}
            </div>
        ...
        {loading ? (
          ...
          <div className="rounded-3xl border border-[#eaded8] bg-white p-3 shadow-[0_16px_45px_rgba(91,17,29,.06)] sm:p-5">
            <MonthCalendar cursor={cursor} onPrev={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() - 1, 1))} onNext={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() + 1, 1))} onToday={() => { const d = new Date(); setCursor(new Date(d.getFullYear(), d.getMonth(), 1)); }} eventsByDay={eventsByDay} onOpen={(id) => navigate({ to: "/admin/$id", params: { id } })} todayISO={todayISO} in7ISO={in7ISO} ordens={ordens} />
          </div>
        )}
      </div>
    </AdminShell>
