...
function OperacaoPage() {
  ...
  const Card = ({ l }: { l: Linha }) => (
    <article className="rounded-2xl border border-[#eaded8] bg-white p-4 shadow-sm">
      ...
      {!l.liberada && <p className="mt-2 text-[11px] text-amber-700">{l.motivo}</p>}
    </article>
  );
      ...
      <div className="mx-auto w-full max-w-[1500px] space-y-6 px-4 py-6 sm:px-6 lg:px-8">
        <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm text-[#7b676a]">Somente contratos com recebimento confirmado entram na operação. Caução permanece informativa e nunca libera compras, produção ou preparação.</p>
          </div>
