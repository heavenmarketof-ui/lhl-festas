...
export const Route = createFileRoute("/orcamento-obrigado")({
  head: () => ({
    meta: [
      ...
      { property: "og:url", content: "https://www.lhlfestas.com.br/orcamento-obrigado" },
    ],
    links: [
      { rel: "canonical", href: "https://www.lhlfestas.com.br/orcamento-obrigado" },
    ],
  }),
  component: Obrigado,
});
...
function Obrigado() {
  ...
  useEffect(() => {
    if (!ready || !qualified || !waUrl) return;
    if (openedKey) {
      try { if (sessionStorage.getItem(openedKey)) return; } catch { /* noop */ }
    }
    const t = setTimeout(() => {
      let win: Window | null = null;
      try { win = window.open(waUrl, "_blank", "noopener"); } catch { win = null; }
      if (win === null) {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center px-6 py-12 text-center">
      ...
      <div className="max-w-xl w-full rounded-3xl bg-card border border-border/60 p-8 sm:p-12 shadow-[var(--shadow-soft)]">
        <div className="mx-auto mb-6 h-12 w-12 rounded-full bg-primary flex items-center justify-center"><PartyPopper className="h-5 w-5 text-primary-foreground" /></div>
        <p className="text-[10px] sm:text-xs uppercase tracking-[0.3em] text-[color:var(--gold)] mb-3">Solicitação recebida</p>
        <h1 className="font-serif text-3xl sm:text-4xl text-primary leading-tight">🎉 Solicitação recebida{primeiroNome ? `, ${primeiroNome}` : ""}!</h1>
        <div className="mx-auto my-6 h-px w-20 bg-gradient-to-r from-transparent via-[color:var(--gold)] to-transparent" />
        {qualified ? <>
          <p className="text-sm sm:text-base text-foreground/85 leading-relaxed">Recebemos sua solicitação com sucesso.<br />Como você já escolheu um tema ou enviou uma imagem de referência, seu atendimento pode ser iniciado <strong>agora mesmo</strong>.</p>
          {popupBlocked && <div className="mt-5 flex items-start gap-2 rounded-xl border border-amber-300 bg-amber-50 p-3 text-left text-xs text-amber-900"><AlertTriangle className="mt-0.5 h-4 w-4 flex-none" /><span>Seu navegador bloqueou a abertura automática. Use o botão abaixo para continuar pelo WhatsApp.</span></div>}
          <p className="mt-4 text-sm text-foreground/80">Clique abaixo para continuar sua conversa pelo WhatsApp.</p>
          <a href={waUrl} target="_blank" rel="noreferrer" onClick={onManualClick} className="mt-4 inline-block w-full"><Button className="h-14 w-full gap-2 text-base bg-[#25D366] text-white hover:bg-[#25D366]/90"><MessageCircle className="h-5 w-5" /> 💬 Continuar atendimento no WhatsApp</Button></a>
          <p className="mt-5 text-xs text-muted-foreground">Se o WhatsApp não abrir automaticamente, use o botão acima.</p>
        </> : <>
          <p className="text-sm sm:text-base text-foreground/85 leading-relaxed">Seu pedido de orçamento já foi encaminhado para nossa equipe.<br />Em breve entraremos em contato pelo WhatsApp informado no formulário.</p>
          <p className="mt-8 text-sm text-foreground/80">Se preferir agilizar seu atendimento, clique abaixo e fale conosco agora mesmo.</p>
          <a href={waUrl} target="_blank" rel="noreferrer" onClick={onManualClick} className="mt-4 inline-block w-full"><Button className="h-14 w-full gap-2 text-base bg-[#25D366] text-white hover:bg-[#25D366]/90"><MessageCircle className="h-5 w-5" /> 💬 Falar no WhatsApp</Button></a>
          <p className="mt-6 text-xs sm:text-sm text-muted-foreground leading-relaxed">Não consegue falar agora? Sem problemas!<br />Sua solicitação já está registrada e nossa equipe entrará em contato em breve.</p>
        </>}
        <Link to="/orcamento" className="mt-6 inline-block text-xs text-muted-foreground hover:text-foreground">Voltar</Link>
      </div>
    </div>
  );
}
